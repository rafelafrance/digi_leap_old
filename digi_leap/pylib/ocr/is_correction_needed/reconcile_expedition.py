...
def classifications_to_points(classifications, increase_by=1):
    ...
    for path, rows in classifications.items():
        for row in rows:
            points[path]["missing"] += get_points(row, " missing:", increase_by)
            points[path]["correct"] += get_points(row, " Correct:", increase_by)
            points[path]["incorrect"] += get_points(row, " Incorrect:", increase_by)
    return points


def get_points(row, label, increase_by):
