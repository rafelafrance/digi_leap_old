...
If you use different number of gpus, the learning rate should be changed to
 0.02/8*$NGPU.
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument(
        ...
        default=0.02,
        type=float,
        help="initial learning rate, 0.02 is the default value for training on "
        "8 gpus and 2 images_per_gpu",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        type=float,
        help="weight decay for Normalization layers (default: None, same "
        "value as --wd)",
    )
    parser.add_argument(
        "--lr-scheduler",
    ...
    parser.add_argument(
        "--use-copypaste",
        action="store_true",
        help="Use CopyPaste data augmentation. Works only with "
...
def main(args):
    ...
    if args.use_copypaste:
        if args.data_augmentation != "lsj":
            raise RuntimeError(
                "SimpleCopyPaste algorithm currently only supports the "
                "'lsj' data augmentation policies"
            )
