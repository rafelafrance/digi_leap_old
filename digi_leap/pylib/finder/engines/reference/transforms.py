    ...
    def forward(
        ...
        if isinstance(image, torch.Tensor):
            if image.ndimension() not in {2, 3}:
                raise ValueError(
                    f"image should be 2/3 dimensional. Got "
                    f"{image.ndimension()} dimensions."
                )
...
class RandomZoomOut(nn.Module):
    ...
    def forward(
        ...
        if isinstance(image, torch.Tensor):
            if image.ndimension() not in {2, 3}:
                raise ValueError(
                    f"image should be 2/3 dimensional. Got {image.ndimension()} "
                    "dimensions."
                )
            elif image.ndimension() == 2:
                image = image.unsqueeze(0)
        ...
        if isinstance(image, torch.Tensor):
            # PyTorch's pad supports only integers on fill.
            # So we need to overwrite the colour
...
class ScaleJitter(nn.Module):
    """Randomly resizes the image and its bounding boxes  within the specified scale
    range. The class implements the Scale Jitter augmentation as described in the paper
    `"Simple Copy-Paste is a Strong Data Augmentation Method for Instance Segmentation"
    <https://arxiv.org/abs/2012.07177>`_.

    Args:
        target_size (tuple of ints): The target size for the transform provided in
        (height, weight) format. scale_range (tuple of ints): scaling factor interval,
        e.g (a, b), then scale is randomly sampled from the range a <= scale <= b.
...
def _copy_paste(
    ...
    if "iscrowd" in target and "iscrowd" in paste_target:
        ...
        # For example, if previous transforms geometrically modifies
        # masks/boxes/labels but
